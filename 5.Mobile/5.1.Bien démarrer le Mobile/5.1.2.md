...
> Utilise **Compose BOM** pour aligner les versions. Remplace `<latest>` par la dernière stable si vous n’utilisez pas un **Version Catalog**.
...
android {
    ...
    composeOptions {
        // Si vous n'avez pas de catalog, décommentez et fixez explicitement la version :
...
## 5) Code Compose de départ (vérification)
...
> ✨ Vous voyez `@Preview` dans l’onglet **Split/Design** → cliquez **Build & Refresh** si besoin.

---

## 6) Exécuter l’app (émulateur ou device)

1. Démarrer un **AVD** (Virtual Device Manager) **ou** brancher un **téléphone** (ADB).
2. Sélectionner la cible dans la barre d’outils.

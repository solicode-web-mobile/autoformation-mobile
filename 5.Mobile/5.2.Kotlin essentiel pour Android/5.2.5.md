---
title: 5. Collections
layout: home
nav_order: 5
parent: 5.2. Kotlin essentiel pour Android
permalink: /kotlin-essentiel-android/collections/
code: 5.2.5
competence: C5
autoformation: "C5.2"
ua: "5.2.U2"
duree_h: 1.5
objectif: "Manipuler List/Set/Map de facon lisible et sure avec les operations idiomatiques."
notions_nouvelles: ["List/Set/Map (immutables/mutables)", "map/filter/mapNotNull", "sumOf/sortedBy", "distinctBy", "groupBy/associateBy", "getOrElse"]
fil_rouge: "App Mobile Compose + API Blog Laravel"
livrable_chapitre: "`filterAndSort`, `asSections`, `stats` + 1–2 tests et exemple Map avec acces sur"
alimentation_prototype: "Listes filtrees/tries pour ecrans (Todo/Articles)"
alimentation_miniprojet: "Sectionnement et agregats (stats UI) performants"
---


# 📘 5.2.5 — `List/Set/Map`

## 📒 Glossaire minute
- **`List<T>`** : liste **ordonnée**, peut contenir des doublons.  
- **`Set<T>`** : ensemble **sans doublons** (égalité structurelle).  
- **`Map<K,V>`** : paires clé→valeur, clés **uniques**.  
- **Immuable** (préféré) : `listOf`, `setOf`, `mapOf`.  
- **Mutable** (à limiter) : `mutableListOf`, `mutableSetOf`, `mutableMapOf`.  
- **Ops usuelles** : `map`, `filter`, `mapNotNull`, `sumOf`, `sortedBy`, `distinctBy`, `groupBy`, `associateBy`.

---

## ✅ Objectif
Manipuler les **collections Kotlin** de manière **lisible et sûre** : choisir le bon type (`List/Set/Map`), **itérer** idiomatiquement, et appliquer les **transformations** clés pour la logique Android/Compose.

---

## 1) Déclarer & choisir la bonne collection

```kotlin
val tags: List<String> = listOf("android", "kotlin", "compose")  // immuable
val bag: MutableList<Int> = mutableListOf(1, 2, 2)               // modifiable

val uniq: Set<String> = setOf("kotlin", "kotlin", "compose")     // {"kotlin","compose"}
val counts: MutableMap<String, Int> = mutableMapOf("done" to 3)
````

> 🌱 **Par défaut, immuable.** Passez au mutable **seulement** si modification nécessaire et **localisée**.

---

## 2) Itérations idiomatiques

```kotlin
// for-each simple
for (t in tags) println(t)

// index + valeur
for ((i, t) in tags.withIndex()) println("$i → $t")

// Map : clé/valeur
for ((k, v) in counts) println("$k = $v")

// Fonctionnel (préféré pour transformer)
tags.forEach { println(it) }
```

> Utilisez `for` si vous avez besoin de `break/continue`. Sinon, préférez les **opérateurs**.

---

## 3) Transformations essentielles

```kotlin
data class Todo(val id: Long, val title: String, val done: Boolean)

val todos = listOf(
  Todo(1, "Étudier Kotlin", true),
  Todo(2, "Écrire UI Compose", false),
  Todo(3, "Tests", true)
)

// Filtrer + projeter
val doneTitles: List<String> =
  todos.filter { it.done }.map { it.title }

// Sommes & comptes
val totalDone = todos.count { it.done }               // 2
val totalChars = todos.sumOf { it.title.length }      // Int
val avgTitle = todos.map { it.title.length }.average()

// Tri & unicité
val sorted = todos.sortedBy { it.title }              // ordre alphabétique
val distinctByTitle = todos.distinctBy { it.title }   // retire doublons par clé

// Groupement & indexation
val byDone: Map<Boolean, List<Todo>> = todos.groupBy { it.done }
val byId: Map<Long, Todo> = todos.associateBy { it.id } // clé = id

// Nettoyage des nulls
val maybeIds: List<Long?> = listOf(1, null, 2)
val ids: List<Long> = maybeIds.filterNotNull()

// Conversion clé/valeur personnalisée
val kv: Map<String, Int> = todos.associate { it.title to it.title.length }
```

---

## 4) `Map` : accès & valeurs par défaut

```kotlin
val m = mapOf("x" to 1)

// Valeur ou défaut calculé
val v1 = m.getOrElse("y") { 0 }       // 0

// Valeur ou constante
val v2 = m["y"] ?: 0                  // 0

// getValue → jette si absent (utile si on **sait** que la clé existe)
val v3 = m.getValue("x")              // 1
```

> **Clés dupliquées** : `toMap()` garde la **dernière** valeur ; préférez `associateBy` si l’**unicité** de la clé est importante.

---

## 5) Recettes Android/Compose

### A) Préparer une liste pour UI (recherche + tri)

```kotlin
fun filterAndSort(todos: List<Todo>, q: String): List<Todo> =
  todos
    .filter { it.title.contains(q.trim(), ignoreCase = true) }
    .sortedBy { it.title.lowercase() }
```

### B) Sectionner une liste (groupBy)

```kotlin
data class Section(val header: String, val items: List<Todo>)

fun asSections(todos: List<Todo>): List<Section> =
  todos.groupBy { if (it.done) "Terminées" else "À faire" }
       .map { (h, items) -> Section(h, items.sortedBy { it.title }) }
```

### C) Compteur & statistiques

```kotlin
data class Stats(val total: Int, val done: Int, val ratio: Double)

fun stats(todos: List<Todo>): Stats {
  val total = todos.size
  val done = todos.count { it.done }
  val ratio = if (total == 0) 0.0 else done.toDouble() / total
  return Stats(total, done, ratio)
}
```

---

## 6) Performance & lisibilité

* Chaînes **longues** d’opérations : utilisez `asSequence()` si la **liste est grosse** et que vous **court-circuitez** (mais souvent **inutile** sur petites listes UI) :

  ```kotlin
  val firstBig = list.asSequence().filter { it.size > 1000 }.map { it.id }.firstOrNull()
  ```
* Construction efficace **immatérielle** :

  ```kotlin
  val built = buildList {
    add(1); addAll(listOf(2,3))
  }
  ```
* Évitez de **muter** une collection en l’**itérant** ; créez une **copie** (`toMutableList()`) si nécessaire.

---

## 7) Cas pratiques supplémentaires

```kotlin
// Limiter résultats & paginer
val page = todos.drop(pageIndex * pageSize).take(pageSize)

// Aplatir des sous-listes
val allTags: List<String> = listOf(
  listOf("android","compose"),
  listOf("kotlin")
).flatMap { it }.distinct()

// Somme de Long/Double
data class Purchase(val priceCents: Long)
val totalCents: Long = listOf(Purchase(1999), Purchase(299)).sumOf { it.priceCents }
```

---

## 🧪 Vérifications rapides

* [ ] Vous distinguez **List/Set/Map** et choisissez l’**immuable** par défaut.
* [ ] Vous savez itérer : `for`, `withIndex`, destructuration `for ((k,v) in map)`.
* [ ] Vous maîtrisez `map`, `filter`, `mapNotNull`, `sumOf`, `sortedBy`.
* [ ] Vous utilisez `groupBy`, `associateBy`, `distinctBy` selon le besoin.

---

## 🩹 Erreurs fréquentes & correctifs

* ❌ Tout en **mutable** → ✅ immuable + **reconstruire** au besoin.
* ❌ `!!` sur valeurs de `Map` → ✅ `getOrElse` / `?:` / `getValue` (si garanti).
* ❌ Tri sensible à la casse → ✅ `sortedBy { it.lowercase() }`.
* ❌ Pipelines trop lourds sur grosses listes → ✅ `asSequence()` ou **algorithme direct**.
* ❌ Perte d’éléments lors d’un `toMap()` avec clés dupliquées → ✅ `groupBy` / `associateBy`.

---

## 📦 Livrables attendus

* Fonctions **utilitaires** :

  * `filterAndSort(todos, q)`,
  * `asSections(todos)`,
  * `stats(todos)`.
* Exemple `Map` avec **accès sûr** (`getOrElse`/`?:`).
* 1–2 tests simple(s) (`sumOf`, `groupBy`, `distinctBy`).

---

## ✅ DoD — Definition of Done

* [ ] Code lisible, **immutabilité** privilégiée.
* [ ] Transformations via **opérateurs** (pas de boucles verbeuses).
* [ ] Accès `Map` **sans NPE**.
* [ ] Exemples compilants & testables.

---

## ⚡ Cheatsheet

```kotlin
// Bases
val l = listOf(1,2,3); val s = setOf(1,1,2); val m = mapOf("a" to 1)

// Itérations
for ((i, v) in l.withIndex()) {}
for ((k, v) in m) {}

// Ops clés
l.filter { it > 1 }.map { it * 10 }.sumOf { it }   // 50
l.distinct().sorted()

// Group & index
val byKey = l.groupBy { it % 2 }      // 0→[2], 1→[1,3]
val index  = todos.associateBy { it.id }

// Map safe
val v = m["x"] ?: 0
```

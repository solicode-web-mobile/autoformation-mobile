...
## 🚀 Préparer votre projet Android Studio

Avant de coder avec Kotlin, créez un projet de base pour cette autoformation :

1. **Ouvrir Android Studio** → *New Project*.
2. Choisir le template **Empty Activity** (Compose support activé si proposé).
3. Donner un nom (ex. `KotlinEssentiel`), package `com.solicode.kotlinessentiel`.
4. Choisir **Langage = Kotlin**, **Minimum SDK = API 21**.
5. Cliquer sur *Finish* et attendre la génération du projet.

### Organiser le code source de la formation
- Créez un dossier `demo/` dans `app/src/main/java/com/solicode/kotlinessentiel/`.
- Chaque chapitre aura un fichier Kotlin dédié (`ValVarDemo.kt`, `NullSafetyDemo.kt`, …).
- Vous pouvez tester vos fonctions soit :
  - via une fonction `main()` (clic droit → *Run*),  
  - soit avec un `@Preview` Compose minimal.

### Exemple rapide : exécuter du code Kotlin
Créez un fichier `ValVarDemo.kt` :
...
➡️ Clic droit sur le fichier → **Run 'ValVarDemoKt'** → sortie dans la console.

...
Écrire du code **sûr** et **lisible** sans `NullPointerException` : choisir correctement `T` vs `T?`, chaîner `?.` + `?:`, utiliser `let`, **éviter `!!`**, et traiter proprement les valeurs optionnelles courantes (saisie utilisateur, conversions numériques, listes).

---

## 1) Choisir `T` ou `T?`

* Par défaut : **non nullable** (`val name: String = "…"`) pour éviter les `null` inutiles.
* Nullable **seulement si** la valeur peut vraiment manquer (`val nickname: String?`).

```kotlin
// Non-nullables (préféré)
...
```kotlin
val name: String? = readLine() // peut être null
...
```kotlin
val s: String? = null
val len = s!!.length // ❌ crash si null
```
...
// 2) Vérifier avant usage (promotion smart-cast)
val t: String? = "Hello"
if (t != null) {
  println(t.length) // t est non nul ici
}

// 3) Exiger explicitement (si vous êtes sûr)
val ok = requireNotNull("Kotlin") { "doit être non null" }
```

> **Règle** : `!!` uniquement si **logiquement impossible** d’être null **ET** déjà vérifié.
...
## 🧪 Vérifications rapides (sans framework de test)

Copiez-collez dans un `fun main()` pour voir le résultat :
...
* ❌ Enchaîner `!!` → ✅ utilisez `?:`, `if (x != null)`, `requireNotNull` au bon endroit.
* ❌ Oublier `toIntOrNull()` → ✅ évitez les `NumberFormatException`.
* ❌ Empiler des `let {}` inutilement → ✅ préférez `?.` + `?:` simples.
...
* Exemples d’utilisation **sans `!!`** (console).
* Un court `main()` qui **affiche** les résultats pour vérification.
...
* [ ] Conversions sûres (`toIntOrNull`) + valeurs par défaut claires.
* [ ] Démonstrations exécutables dans un `main()`.

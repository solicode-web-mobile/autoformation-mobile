...
* Requêtes CRUD exécutées et testées dans **Tinker**.
* Manipulation des relations (`attach`, `sync`, `detach`).
* Capture d’écran des résultats de `Article::count()` et `Article::with('tags')->get()`.

---

## 🔗 Suite du parcours

> Prochain module → **2.2. API & Contrôleurs REST**
> Vous apprendrez à **exposer vos données** via des routes REST et à **renvoyer des réponses JSON** structurées.
